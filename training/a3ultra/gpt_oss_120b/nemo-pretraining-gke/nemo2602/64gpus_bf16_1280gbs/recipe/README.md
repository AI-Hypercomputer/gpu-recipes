<!-- mdformat global-off -->
# Pretrain gpt-oss-120b workloads on a3ultra GKE Node pools with Nvidia Megatron-Bridge Framework
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a3ultra-gpt-oss-120b-8node
...
```
kubectl get pods | grep $USER-a3ultra-gpt-oss-120b-8node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a3ultra-gpt-oss-120b-8node.
...
For example: `$USER-a3ultra-gpt-oss-120b-8node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a3ultra-gpt-oss-120b-8node
```
