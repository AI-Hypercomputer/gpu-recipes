<!-- mdformat global-off -->
# Pretrain qwen3-30b-a3b workloads on a3ultra GKE Node pools with Nvidia Megatron-Bridge Framework
...
#### Using 2 node (16 gpus) fp8cs precision
To execute the job with the default settings, run the following command from
your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a3ultra-qwen3-30b-a3b-2node
...
```
kubectl get pods | grep $USER-a3ultra-qwen3-30b-a3b-2node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a3ultra-qwen3-30b-a3b-2node.
...
For example: `$USER-a3ultra-qwen3-30b-a3b-2node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a3ultra-qwen3-30b-a3b-2node
```
