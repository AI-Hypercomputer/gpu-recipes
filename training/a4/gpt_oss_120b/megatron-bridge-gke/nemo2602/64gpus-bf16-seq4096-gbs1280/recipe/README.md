<!-- mdformat global-off -->
# Pretrain gptoss-120b-bf16-gbs1280-gpus64 workloads on a4 GKE Node pools with Megatron-Bridge

This recipe outlines the steps for running a gptoss-120b pretraining
...
- Node Configuration: 8 nodes (8 GPUs per node, 64 GPUs total).
...
export RECIPE_ROOT=$REPO_ROOT/training/a4/gpt_oss_120b/megatron-bridge-gke/nemo2602/64gpus-bf16-seq4096-gbs1280/recipe
cd $RECIPE_ROOT
```

### Configure and submit a pretraining job

#### Using 8 nodes (64 gpus) bf16 precision

To execute the job with the default settings, run the following command from your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-gptoss-120b-8node-bf16-gbs1280
...
```
kubectl get pods | grep $USER-gptoss-120b-8node-bf16-gbs1280
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-gptoss-120b-8node-bf16-gbs1280.
...
For example: `$USER-gptoss-120b-8node-bf16-gbs1280-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-gptoss-120b-8node-bf16-gbs1280
```