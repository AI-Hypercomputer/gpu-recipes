<!-- mdformat global-off -->
# Pretrain qwen2-5-7b workloads on a4 GKE Node pools with Nvidia NeMo Framework

This recipe outlines the steps for running a qwen2-5-7b pretraining
...
export RECIPE_ROOT=$REPO_ROOT/training/a4/qwen2-5-7b/nemo-pretraining-gke/2node-bf16-seq8192-gbs256/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4-qwen2-5-7b-2node
helm install $WORKLOAD_NAME . -f values.yaml \
--set-file workload_launcher=launcher.sh \
--set-file workload_config=qwen2-5-7b-bf16-seq8192-gbs256-gpus8.py \
...
```
kubectl get pods | grep $USER-a4-qwen2-5-7b-2node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4-qwen2-5-7b-2node.
...
For example: `$USER-a4-qwen2-5-7b-2node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4-qwen2-5-7b-2node
```
