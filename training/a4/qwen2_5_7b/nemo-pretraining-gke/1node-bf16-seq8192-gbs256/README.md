...
export RECIPE_ROOT=$REPO_ROOT/training/a4/qwen2-5-7b/nemo-pretraining-gke/1node-bf16-seq8192-gbs256/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4-qwen2-5-7b-1node
...
```
kubectl get pods | grep $USER-a4-qwen2-5-7b-1node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4-qwen2-5-7b-1node.
...
For example: `$USER-a4-qwen2-5-7b-1node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4-qwen2-5-7b-1node
```
