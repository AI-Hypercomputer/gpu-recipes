...
export RECIPE_ROOT=$REPO_ROOT/training/a4/llama3-1-70b/nemo-pretraining-gke/2_nodes
cd $RECIPE_ROOT
```
...
your client:
    ...
    ```bash
    cd $RECIPE_ROOT
    export WORKLOAD_NAME=$USER-a4-llama3-1-70b-2node
    helm install $WORKLOAD_NAME . -f values.yaml \
    --set-file workload_launcher=launcher.sh \
    --set-file workload_config=llama3-1-70b-seq8192-gbs256-mbs1-gpus16.py \
...
```
kubectl get pods | grep $USER-a4-llama3-1-70b-2node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4-llama3-1-70b-2node.
...
For example: `$USER-a4-llama3-1-70b-2node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4-llama3-1-70b-2node
```