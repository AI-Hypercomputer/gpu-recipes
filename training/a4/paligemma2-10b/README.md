# Finetune PaliGemma2 10B workloads on A4 GKE Node pools

This recipe outlines the steps for running a PaliGemma2 finetuning workload on
...
- Finetuning job configuration and deployment - A Helm chart is used to configure and deploy
  the [Kubernetes Job](https://kubernetes.io/docs/concepts/workloads/controllers/job/)
  resource which manages the execution  of the workload.
...
This recipe uses [Waymo Perception dataset V2.0.1](https://console.cloud.google.com/storage/browser/waymo_open_dataset_v_2_0_1). The dataset is preprocessed to contain image and object information only to reduce data processing time during training.
...
This image contains the NCCL gIB plugin v1.0.5, bundling all NCCL binaries validated for use with A4 GPUs.
...
export RECIPE_ROOT=$REPO_ROOT/training/a4/paligemma2-10b/
cd $RECIPE_ROOT
```
...
### Configure and submit a finetuning job
