"""Nemo2 pretraining recipe for Qwen2.5 7B model."""

from nemo.collections import llm
from nemo.collections.llm.recipes import qwen25_7b
...
def recipe(
  ...
  """Returns a Nemo2 training recipe for Qwen2.5 7B model.
  ...
  pretrain = qwen25_7b.pretrain_recipe()

  # Enable fused operations for performance.
  pretrain.model.config.cross_entropy_fusion_impl = "te"
  pretrain.model.config.cross_entropy_loss_fusion = True
  pretrain.model.config.apply_rope_fusion = True
  pretrain.model.config.bias_dropout_fusion = True
  pretrain.model.config.bias_activation_fusion = True

  # Disable activation recomputation to maximize throughput (if memory permits).
  ...
  # We use 'qwen3' prefix to ensure NeMo 2 uses the correct Qwen-family FLOPs formula.
  pretrain.trainer.callbacks.append(
      run.Config(
          FLOPsMeasurementCallback,
          model_name="qwen3-qwen25-7b",
          model_config=pretrain.model.config,
          data_config=pretrain.data,
      )
  )

  # For Qwen 2.5 7B, performance_mode is not supported in the base recipe.
  # We manually check for or add the Megatron communication overlap callback if needed.
