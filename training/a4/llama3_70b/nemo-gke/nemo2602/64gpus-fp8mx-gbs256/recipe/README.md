<!-- mdformat global-off -->
# Pretrain llama3-70b workloads on a4 GKE Node pools with Nvidia Megatron-Bridge Framework
...
export RECIPE_ROOT=$REPO_ROOT/training/a4/llama3_70b/nemo-gke/nemo2602/64gpus-fp8mx-gbs256/recipe
cd $RECIPE_ROOT
```
...
#### Using 8 node (64 gpus) fp8 precision
To execute the job with the default settings, run the following command from
your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4-llama3-70b-8node
...
```
kubectl get pods | grep $USER-a4-llama3-70b-8node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4-llama3-70b-8node.
...
For example: `$USER-a4-llama3-70b-8node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4-llama3-70b-8node
```
