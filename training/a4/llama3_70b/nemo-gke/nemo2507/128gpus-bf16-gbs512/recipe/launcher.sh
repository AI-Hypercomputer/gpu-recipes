...
if [[ -n "${RESTORE_PATH}" ]]; then
  RESTORE_ARG="restore_path=\"${RESTORE_PATH}\","
else
  RESTORE_ARG=""
fi
...
if [[ -n "${TOKEN_PATH}" ]]; then
  TOKEN_ARG="token_path=\"${TOKEN_PATH}\","
else
  TOKEN_ARG=""
fi
...
python ${NEMO_LAUNCH_SCRIPT} --factory "recipe(job_name=${JOB_IDENTIFIER}, \
enable_dataloading=${ENABLE_DATALOADING}, \
enable_ckpt_write=${ENABLE_CKPT_WRITE}, \
enable_ckpt_load=${ENABLE_CKPT_LOAD}, \
${RESTORE_ARG} \
${DATASET_ARG} \
${TOKEN_ARG} \
ckpt_write_interval=${CKPT_WRITE_INTERVAL})" \
trainer.num_nodes="$NNODES" \
log.explicit_log_dir="${explicit_log_dir}" \
trainer.max_steps=${STEP_COUNT} trainer.devices="${GPUS_PER_NODE}" \
...
${NEMO_LAUNCH_SCRIPT} --factory "recipe(job_name=${JOB_IDENTIFIER}, \
enable_dataloading=${ENABLE_DATALOADING}, \
enable_ckpt_write=${ENABLE_CKPT_WRITE}, \
enable_ckpt_load=${ENABLE_CKPT_LOAD}, \
${RESTORE_ARG} \
...
if [[ "$JOB_COMPLETION_INDEX" == "0" ]]; then
  ...
  mkdir -p ${ARTIFACT_DIR}/dllogger/rank-0
  ...
  cp dllogger.json ${ARTIFACT_DIR}/rank-0/dllogger.json
