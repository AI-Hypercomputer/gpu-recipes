...
helm  install -f $RECIPE_ROOT/values.yaml \
    ...
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/maxtext-configs/llama3-1-405b-256gpus-a4-fp8.yaml \
...
helm  install -f $RECIPE_ROOT/values.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/maxtext-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/maxtext-configs/llama3-1-405b-256gpus-a4-bf16.yaml \
    --set workload.gpus=256 \
    --set queue=$KUEUE_NAME \
    --set workload.arguments[0]="base_output_directory=gs://${GCS_BUCKET}/maxtext-experiments" \
...
helm  install -f $RECIPE_ROOT/values.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/maxtext-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/maxtext-configs/llama3-1-405b-1024gpus-a4-fp8.yaml \
...
helm  install -f $RECIPE_ROOT/values.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/maxtext-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/maxtext-configs/llama3-1-405b-1024gpus-a4-bf16.yaml \
    --set workload.gpus=1024 \
    --set queue=$KUEUE_NAME \
    --set workload.arguments[0]="base_output_directory=gs://${GCS_BUCKET}/maxtext-experiments" \
    $USER-llama-3-1-405b-maxtext-bf16 \
...
- JOB_NAME_PREFIX - your job name prefix. For example $USER-llama-3-1-405b-maxtext.
...
`JOB_NAME_PREFIX-workload-0-0`. For example: `user-llama-3-1-405b-maxtext-workload-0-0-s9zrv`.
...
- `CLUSTER_REGION`: the region where your cluster is located.
- `CLUSTER_NAME`: the name of your GKE cluster.
- `JOB_NAME_PREFIX`: the prefix of your job name (e.g., `$USER-llama-3-1-405b-maxtext`).
