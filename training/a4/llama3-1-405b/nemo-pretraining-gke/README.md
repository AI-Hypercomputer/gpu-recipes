...
helm  install -f $RECIPE_ROOT/values.yaml \
    ...
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/nemo-configs/llama3-1-405b-224gpus-a4-fp8.yaml \
...
helm  install -f $RECIPE_ROOT/values.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/nemo-10-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/nemo-configs/llama3-1-405b-224gpus-a4-bf16.yaml \
    --set queue=${KUEUE_NAME} \
    --set volumes.gcsMounts[0].bucketName=${GCS_BUCKET} \
...
```bash
helm  install -f $RECIPE_ROOT/values-56-112-nodes.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/nemo-10-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/nemo-configs/llama3-1-405b-448gpus-a4-fp8.yaml \
    --set queue=${KUEUE_NAME} \
    --set volumes.gcsMounts[0].bucketName=${GCS_BUCKET} \
    $USER-llama-3-1-405b-nemo-fp8 \
    $REPO_ROOT/src/helm-charts/a4/jobset
...
helm  install -f $RECIPE_ROOT/values-56-112-nodes.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/nemo-10-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/nemo-configs/llama3-1-405b-448gpus-a4-bf16.yaml \
    --set queue=${KUEUE_NAME} \
    --set volumes.gcsMounts[0].bucketName=${GCS_BUCKET} \
    $USER-llama-3-1-405b-nemo-bf16 \
    $REPO_ROOT/src/helm-charts/a4/jobset
...
#### Using 112 nodes (896 GPUs) FP8 precision
...
helm  install -f $RECIPE_ROOT/values-56-112-nodes.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/nemo-10-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/nemo-configs/llama3-1-405b-896gpus-a4-fp8.yaml \
    --set queue=${KUEUE_NAME} \
    --set volumes.gcsMounts[0].bucketName=${GCS_BUCKET} \
    --set workload.gpus=896 \
    $USER-llama-3-1-405b-nemo-fp8 \
    $REPO_ROOT/src/helm-charts/a4/jobset
```

#### Using 112 nodes (896 GPUs) BF16 precision
...
helm  install -f $RECIPE_ROOT/values-56-112-nodes.yaml \
    --set-file workload_launcher=$REPO_ROOT/src/launchers/nemo-10-launcher.sh \
    --set-file workload_config=$REPO_ROOT/src/frameworks/a4/nemo-configs/llama3-1-405b-896gpus-a4-bf16.yaml \
    --set queue=${KUEUE_NAME} \
    --set volumes.gcsMounts[0].bucketName=${GCS_BUCKET} \
    --set workload.gpus=896 \
    $USER-llama-3-1-405b-nemo-bf16 \
...
For example: `user-llama-3-1-405b-nemo-fp8-workload-0-0-s9zrv`.
...
$USER-llama-3-1-405b-nemo-[YYYY]-[MM]-[DD]-[hh]-[mm]-[ss], where the suffix of the ID is a day and time when the job was started.
...
- `CLUSTER_REGION`: the region where your cluster is located.
- `CLUSTER_NAME`: the name of your GKE cluster.
- `JOB_NAME_PREFIX`: the prefix of your job name (e.g., `$USER-llama-3-1-405b-nemo`).
