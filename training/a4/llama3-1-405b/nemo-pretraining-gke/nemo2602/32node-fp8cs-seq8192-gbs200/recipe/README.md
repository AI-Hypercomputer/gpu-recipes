...
#### Using 32 node (256 gpus) fp8cs precision
To execute the job with the default settings, run the following command from
your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4-llama31-405b-32node
...
```
kubectl get pods | grep $USER-a4-llama31-405b-32node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4-llama31-405b-32node.
...
For example: `$USER-a4-llama31-405b-32node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4-llama31-405b-32node
```
