...
export RECIPE_ROOT=$REPO_ROOT/training/a4x/llama31_405b/megatron-bridge-gke/nemo2602/256gpus-fp8mx-seq8192-gbs1536/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-llama31-405b-64node
...
```
kubectl get pods | grep $USER-a4x-llama31-405b-64node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-llama31-405b-64node.
...
For example: `$USER-a4x-llama31-405b-64node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-llama31-405b-64node
```
