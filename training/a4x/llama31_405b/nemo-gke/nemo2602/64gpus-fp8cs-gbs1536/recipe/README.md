<!-- mdformat global-off -->
# Pretrain llama31-405b workloads on a4x GKE Node pools with Nvidia Megatron-Bridge Framework
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x/llama31_405b/nemo-gke/nemo2602/64gpus-fp8cs-gbs1536/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-llama31-405b-16node
...
-   To set the number of training steps to 100, run the following command from
    ...
    --set workload.envs[0].value=/job-logs/$WORKLOAD_NAME \
    --set queue=${KUEUE_NAME} \
    --set workload.arguments[0]="trainer.max_steps=30"
    ```
...
```
kubectl get pods | grep $USER-a4x-llama31-405b-16node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-llama31-405b-16node.
...
For example: `$USER-a4x-llama31-405b-16node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-llama31-405b-16node
```
