<!-- mdformat global-off -->
# Pretrain llama3-70b workloads on a4x GKE Node pools with Nvidia Megatron-Bridge Framework
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-llama3-70b-16node
...
```
kubectl get pods | grep $USER-a4x-llama3-70b-16node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-llama3-70b-16node.
...
For example: `$USER-a4x-llama3-70b-16node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-llama3-70b-16node
```
