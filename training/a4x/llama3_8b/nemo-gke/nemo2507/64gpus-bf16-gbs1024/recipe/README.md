<!-- mdformat global-off -->
# Pretrain llama3-1-8b workloads on a4x GKE Node pools with Nvidia NeMo Framework
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x/llama3_8b/nemo-gke/nemo2507/64gpus-bf16-gbs1024/recipe
cd $RECIPE_ROOT
```
...
#### Using 16 node (64 gpus) bf16 precision
...
```
kubectl get pods | grep $USER-a4x-llama3-1-8b-16node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-llama3-1-8b-16node.
...
For example: `$USER-a4x-llama3-1-8b-16node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-llama3-1-8b-16node
```