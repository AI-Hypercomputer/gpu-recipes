<!-- mdformat global-off -->
# Pretrain wan2-1-14b-bf16-gbs128-gpus64 workloads on a4x GKE Node pools with NVIDIA DFM & Megatron-Bridge

This recipe outlines the steps for running a wan2-1-14b-bf16-gbs128-gpus64 pretraining workload on a4x GKE Node pools by using the NeMo DFM (Diffusion Foundation Models) and Megatron-Bridge within Nemo Framework.
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x/wan_14b/nemo-gke/nemo2602/64gpus-bf16-gbs128/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-wan2-1-14b-16node
...
-   To set the number of training steps to 100, run the following command from
    ...
    --set workload.envs[0].value=/job-logs/$WORKLOAD_NAME \
    --set queue=${KUEUE_NAME} \
    --set workload.arguments[0]="train.train_iters=30"
    ```
...
```
kubectl get pods | grep $USER-a4x-wan2-1-14b-16node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-wan2-1-14b-16node.
...
For example: `$USER-a4x-wan2-1-14b-16node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-wan2-1-14b-16node
```