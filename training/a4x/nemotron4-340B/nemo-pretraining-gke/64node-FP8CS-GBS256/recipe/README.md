...
export RECIPE_ROOT=$REPO_ROOT/training/a4x/nemotron4-340B/nemo-pretraining-gke/64node-FP8CS-GBS256/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-nemotron4-340b-64node
helm install $WORKLOAD_NAME . -f values.yaml \
--set-file workload_launcher=launcher.sh \
--set-file workload_config=nemotron4-340b-fp8cs-gbs256-gpus256.py \
...
```
kubectl get pods | grep $USER-a4x-nemotron4-340b-64node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-nemotron4-340b-64node.
...
For example: `$USER-a4x-nemotron4-340b-64node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-nemotron4-340b-64node
```