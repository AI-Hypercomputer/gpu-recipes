...
worker_command=$(cat <<- EOM
  ...
  python scripts/performance/run_script.py \
    ...
    ++model.cutedsl_fused_grouped_mlp=true
...
if [[ "$JOB_COMPLETION_INDEX" == "0" && -n "${ARTIFACT_DIR}" ]]; then
  if mkdir -p "${ARTIFACT_DIR}" 2>/dev/null; then
    ...
    env > "${ARTIFACT_DIR}/environ.txt" 2>/dev/null || true
    ls "${ARTIFACT_DIR}" 2>/dev/null || true
  fi
fi

# All nodes copy their local nsys profiles to GCS
if [[ -d "/tmp/nsys" && -n "${ARTIFACT_DIR}" ]]; then
  if mkdir -p "${ARTIFACT_DIR}/nsys" 2>/dev/null; then
    cp -f /tmp/nsys/*.nsys-rep "${ARTIFACT_DIR}/nsys/" 2>/dev/null || true
    cp -f /tmp/nsys/*.sqlite "${ARTIFACT_DIR}/nsys/" 2>/dev/null || true
  fi
fi
