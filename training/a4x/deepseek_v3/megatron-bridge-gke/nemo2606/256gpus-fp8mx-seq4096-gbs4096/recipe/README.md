<!-- mdformat global-off -->
# Pretrain deepseek-v3 workloads on a4x GKE Node pools with Nvidia Megatron-Bridge Framework
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x/deepseek_v3/megatron-bridge-gke/nemo2606/256gpus-fp8mx-seq4096-gbs4096/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-deepseek-v3-64node
...
```
kubectl get pods | grep $USER-a4x-deepseek-v3-64node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-deepseek-v3-64node.
...
For example: `$USER-a4x-deepseek-v3-64node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-deepseek-v3-64node
```
