<!-- mdformat global-off -->
# Pretrain qwen3-235b-a22b workloads on a4x_max GKE Node pools with Nvidia Megatron-Bridge Framework
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x_max/qwen3-235b-a22b/megatron-bridge-pretraining-gke/32node-FP8MX-GBSunknown/recipe
cd $RECIPE_ROOT
```
...
#### Using 32 node (128 gpus) fp8mx precision
To execute the job with the default settings, run the following command from
your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x_max-qwen3-235b-a22b-32node
...
```
kubectl get pods | grep $USER-a4x_max-qwen3-235b-a22b-32node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x_max-qwen3-235b-a22b-32node.
...
For example: `$USER-a4x_max-qwen3-235b-a22b-32node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x_max-qwen3-235b-a22b-32node
```
