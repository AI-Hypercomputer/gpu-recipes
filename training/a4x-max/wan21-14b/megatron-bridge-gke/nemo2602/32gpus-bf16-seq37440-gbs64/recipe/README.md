...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/wan21-14b/megatron-bridge-gke/nemo2602/32gpus-bf16-seq37440-gbs64/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-wan21-14b-32gpus
...
```
kubectl get pods | grep $USER-a4x-max-wan21-14b-32gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-wan21-14b-32gpus.
...
For example: `$USER-a4x-max-wan21-14b-32gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-wan21-14b-32gpus
```
