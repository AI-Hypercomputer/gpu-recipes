...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/wan21-14b/megatron-bridge-gke/nemo2602/64gpus-bf16-seq37440-gbs128/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-wan21-14b-64gpus
...
```
kubectl get pods | grep $USER-a4x-max-wan21-14b-64gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-wan21-14b-64gpus.
...
For example: `$USER-a4x-max-wan21-14b-64gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-wan21-14b-64gpus
```
