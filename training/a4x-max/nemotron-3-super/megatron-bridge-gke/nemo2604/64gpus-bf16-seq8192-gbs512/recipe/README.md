<!-- mdformat global-off -->
# Pretrain nemotron_3_super workloads on a4x-max GKE Node pools with Nvidia Megatron-Bridge Framework

This recipe outlines the steps for running a nemotron_3_super pretraining
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/nemotron-3-super/megatron-bridge-gke/nemo2604/64gpus-bf16-seq8192-gbs512/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-nemotron_3_super-64gpus
helm install $WORKLOAD_NAME . -f values.yaml \
--set-file workload_launcher=launcher.sh \
--set-file workload_config=custom_setup_experiment.py \
--set workload.image=nvcr.io/nvidia/nemo:26.04 \
...
```
kubectl get pods | grep $USER-a4x-max-nemotron_3_super-64gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-nemotron_3_super-64gpus.
...
For example: `$USER-a4x-max-nemotron_3_super-64gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-nemotron_3_super-64gpus
```
