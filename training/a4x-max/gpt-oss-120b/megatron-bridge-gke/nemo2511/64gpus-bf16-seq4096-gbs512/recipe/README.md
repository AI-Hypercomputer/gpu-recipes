<!-- mdformat global-off -->
# Pretrain gpt_oss_120b workloads on a4x-max GKE Node pools with Nvidia Megatron-Bridge Framework

This recipe outlines the steps for running a gpt_oss_120b pretraining
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/gpt-oss-120b/megatron-bridge-gke/nemo2511/64gpus-bf16-seq4096-gbs512/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-gpt-oss-120b-64gpus
...
```
kubectl get pods | grep $USER-gpt-oss-120b-64gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-gpt-oss-120b-64gpus.
...
For example: `$USER-gpt-oss-120b-64gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-gpt-oss-120b-64gpus
```
