...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/gpt-oss-120b/megatron-bridge-gke/nemo2511/128gpus-bf16-seq4096-gbs512/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-gpt-oss-120b-128gpus
...
```
kubectl get pods | grep $USER-gpt-oss-120b-128gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-gpt-oss-120b-128gpus.
...
For example: `$USER-gpt-oss-120b-128gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-gpt-oss-120b-128gpus
```
