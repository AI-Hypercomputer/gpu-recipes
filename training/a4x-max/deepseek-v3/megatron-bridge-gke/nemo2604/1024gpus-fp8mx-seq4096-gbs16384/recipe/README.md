...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/deepseek-v3/megatron-bridge-gke/nemo2604/1024gpus-fp8mx-seq4096-gbs16384/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-deepseek-v3-1024gpus
...
```
kubectl get pods | grep $USER-a4x-max-deepseek-v3-256node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-deepseek-v3-256node.
...
For example: `$USER-a4x-max-deepseek-v3-256node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-deepseek-v3-256node
```
