...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/deepseek-v3/megatron-bridge-gke/nemo2604/512gpus-fp8mx-seq4096-gbs8192/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-deepseek-v3-512gpus
...
```
kubectl get pods | grep $USER-a4x-max-deepseek-v3-128node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-deepseek-v3-128node.
...
For example: `$USER-a4x-max-deepseek-v3-128node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-deepseek-v3-128node
```
