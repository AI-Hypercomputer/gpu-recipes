...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/deepseek-v3/megatron-bridge-gke/nemo2606/256gpus-fp8mx-seq4096-gbs4096/recipe
cd $RECIPE_ROOT
```
...
--set workload.image=nvcr.io/nvidia/nemo:26.06.00 \
...
```
kubectl get pods | grep $USER-a4x-max-deepseek-v3-64node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-deepseek-v3-64node.
...
For example: `$USER-a4x-max-deepseek-v3-64node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-deepseek-v3-64node
```
