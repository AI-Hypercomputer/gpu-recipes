<!-- mdformat global-off -->
# Pretrain deepseek_v3 workloads on a4x-max GKE Node pools with Nvidia Megatron-Bridge Framework
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/deepseek-v3/megatron-bridge-gke/nemo2511/256gpus-bf16-seq4096-gbs2048/recipe
cd $RECIPE_ROOT
```
...
```
kubectl get pods | grep $USER-deepseek-v3-256gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-deepseek-v3-256gpus.
...
For example: `$USER-deepseek-v3-256gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-deepseek-v3-256gpus
```
