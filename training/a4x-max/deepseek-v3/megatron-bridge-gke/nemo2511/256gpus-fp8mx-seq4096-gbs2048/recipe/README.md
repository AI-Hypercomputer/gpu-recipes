<!-- mdformat global-off -->
# Pretrain deepseek-v3 workloads on a4x-max GKE Node pools with Nvidia Megatron-Bridge Framework
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/deepseek-v3/megatron-bridge-gke/nemo2511/256gpus-fp8mx-seq4096-gbs2048/recipe
cd $RECIPE_ROOT
```
...
#### Using 256 GPUs fp8mx precision
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-deepseek-v3-256gpus
...
```
kubectl get pods | grep $USER-a4x-max-deepseek-v3-256gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-deepseek-v3-256gpus.
...
For example: `$USER-a4x-max-deepseek-v3-256gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-deepseek-v3-256gpus
```
