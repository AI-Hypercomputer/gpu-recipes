...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/kimi-k2/megatron-bridge-gke/nemo2606/256gpus-fp8mx-seq4096-gbs4096/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-kimi-k2-256gpus
...
```
kubectl get pods | grep $USER-a4x-max-kimi-k2-256gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-kimi-k2-256gpus.
...
For example: `$USER-a4x-max-kimi-k2-256gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-kimi-k2-256gpus
```
