<!-- mdformat global-off -->
# Pretrain llama31-405b workloads on a4x_max GKE Node pools with Nvidia Megatron-Bridge Framework

This recipe outlines the steps for running a llama31-405b pretraining
workload on [a4x_max GKE Node pools](https://cloud.google.com/kubernetes-engine) by using the
...
to create your a4x_max GKE cluster.
...
Replace the following values:
 ...
 - `<KUEUE_NAME>`: the name of the Kueue local queue. The default queue created by the cluster toolkit is `a4x_max`. Make sure to verify the name of the local queue in your cluster.
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x_max/llama31-405b/megatron-bridge-pretraining-gke/64node-FP8CS-GBSunknown/recipe
cd $RECIPE_ROOT
```
...
#### Using 64 node (256 gpus) fp8cs precision
To execute the job with the default settings, run the following command from
your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x_max-llama31-405b-64node
...
```
kubectl get pods | grep $USER-a4x_max-llama31-405b-64node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x_max-llama31-405b-64node.
...
For example: `$USER-a4x_max-llama31-405b-64node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x_max-llama31-405b-64node
```
