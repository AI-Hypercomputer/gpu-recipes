...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/llama3-1-405b/megatron-bridge-gke/nemo2604/512gpus-fp8mx-seq8192-gbs3072/recipe
cd $RECIPE_ROOT
```
...
#### Using 512 gpus fp8mx precision
To execute the job with the default settings, run the following command from
your client:

```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-llama3-1-405b-512gpus
...
```
kubectl get pods | grep $USER-a4x-max-llama3-1-405b-512gpus
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-llama3-1-405b-512gpus.
...
For example: `$USER-a4x-max-llama3-1-405b-512gpus-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-llama3-1-405b-512gpus
```
