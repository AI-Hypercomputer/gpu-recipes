...
worker_command=$(cat <<- EOM
  ...
  nvidia-smi dmon -s pum -o DT --id \$gpu_uuid --format csv --gpm-metrics 1,2,3,5,10,20,21,60,61,100,101,102,103 -f \$telemetry_dir/\$RANK.csv &
  nv_smi_pid=\$!
  ...
  echo "Killing nvidia-smi dmon: \$nv_smi_pid"
  kill -9 \$nv_smi_pid
EOM
)
