...
- `nccl-gib-plugins_1.1.2-1_arm64.deb`
...
export RECIPE_ROOT=$REPO_ROOT/training/a4x-max/llama3-1-405b/megatron-bridge-gke/nemo2604/1024gpus-fp8mx-seq8192-gbs6144/recipe
cd $RECIPE_ROOT
```
...
```bash
cd $RECIPE_ROOT
export WORKLOAD_NAME=$USER-a4x-max-llama3-1-405b-1024gpus
...
```
kubectl get pods | grep $USER-a4x-max-llama3-1-405b-256node
```

Replace the following:

- JOB_NAME_PREFIX - your job name prefix. For example $USER-a4x-max-llama3-1-405b-256node.
...
For example: `$USER-a4x-max-llama3-1-405b-256node-workload-0-0-s9zrv`.
...
```bash
helm uninstall $USER-a4x-max-llama3-1-405b-256node
```
