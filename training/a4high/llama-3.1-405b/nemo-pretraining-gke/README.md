# Pretrain Llama-3.1-405B workloads on A4 High GKE Node pools with Nvidia NeMo Framework
...
- A cluster with 28 [a4-highgpu-8g](https://cloud.google.com/compute/docs/accelerator-optimized-machines#a4-high-vms) machines
...
- [NVIDIA NeMo NGC container image](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/nemo/tags): 25.02
...
- A GKE cluster with the following setup:
    - An A4 High node pool (28 nodes, 224 GPUs)
...
1. Set the environment variables to match your environment:
  ...
  - `<KUEUE_NAME>`: the name of the Kueue queue configured for TAS. The default queue created by the cluster toolkit is `a4-high`. Please check it that is the name of your local queue by running `kubectl get queues` and modify accordingly.
...
export RECIPE_ROOT=$REPO_ROOT/training/a4high/llama-3.1-405b/nemo-pretraining-gke
```
...
This recipe uses the following docker image: `us-central1-docker.pkg.dev/deeplearning-images/reproducibility/pytorch-gpu-nemo-nccl:nemo25.02-gib1.0.5-A4`.

This image is based on NVIDIA NeMo 25.02 and contains the NCCL gIB plugin v1.0.5, bundling all NCCL binaries validated for use with A4 High GPUs.
...
helm  install -f values.yaml \
    --set-file nemo_config=$REPO_ROOT/src/frameworks/a4high/nemo-configs/llama-3.1-405b-224gpus-a4high-fp8.yaml \
    ...
    $REPO_ROOT/src/helm-charts/a4high/nemo-training
```
...
cd $RECIPE_ROOT
helm  install -f values.yaml \
    --set-file nemo_config=$REPO_ROOT/src/frameworks/a4high/nemo-configs/llama-3.1-405b-224gpus-a4high-bf16.yaml \
    --set workload.image=us-central1-docker.pkg.dev/deeplearning-images/reproducibility/pytorch-gpu-nemo-nccl:nemo25.02-gib1.0.5-A4 \
    --set queue=${KUEUE_NAME} \
    --set volumes.gcsMounts[0].bucketName=${GCS_BUCKET} \
...
[NeMo configurations fp8](../../../../src/frameworks/a4high/nemo-configs/llama-3.1-405b-224gpus-a4high-fp8.yaml)
[NeMo configurations bf16](../../../../src/frameworks/a4high/nemo-configs/llama-3.1-405b-224gpus-a4high-bf16.yaml)
...
- `hparams.yaml`: the NeMo configuration used by the pretraining script. This includes
   the combined [configuration file](../../../../src/frameworks/a4high/nemo-configs/llama-3.1-405b-224gpus-a4high-fp8.yaml)
