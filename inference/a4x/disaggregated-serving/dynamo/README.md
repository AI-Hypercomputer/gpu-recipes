...
Frontend logs:
```bash
kubectl logs -f deployment/$USER-dynamo-a4x-multi-node-frontend -n ${NAMESPACE}
```

Decode worker logs:
```bash
kubectl logs -f deployment/$USER-dynamo-a4x-multi-node-decode-worker -n ${NAMESPACE}
```

Prefill worker logs:
```bash
kubectl logs -f deployment/$USER-dynamo-a4x-multi-node-prefill-worker -n ${NAMESPACE}
```
...
```bash
helm uninstall $USER-dynamo-a4x-multi-node -n ${NAMESPACE}
```
