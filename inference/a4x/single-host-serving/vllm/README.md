# Single Host Model Serving with vLLM on A4X GKE Node Pool

This document outlines the steps to serve and benchmark various Large Language Models (LLMs) using the [vLLM](https://github.com/vllm-project/vllm) framework on a single [A4X GKE Node pool](https://cloud.google.com/kubernetes-engine).
...
* [4. Run the Recipe](#run-the-recipe)
  * [4.1. Serving and Inference Benchmarking DeepSeek-R1 671B Model](#serving-deepseek-r1-671b)
      * [4.1.1. Deploy DeepSeek-R1 671B Model](#deploy-deepseek-r1-671b)
      * [4.1.2. Interact with DeepSeek-R1 671B Model](#interact-with-deepseek-r1-671b)
      * [4.1.3. Benchmark DeepSeek-R1 671B Model](#benchmark-deepseek-r1-671b)
...
export RECIPE_ROOT=$REPO_ROOT/inference/a4x/single-host-serving/vllm
```
...
export CLUSTER_REGION=<REGION_OF_YOUR_GKE_CLUSTER>
export CLUSTER_NAME=<NAME_OF_YOUR_GKE_CLUSTER>
export KUEUE_NAME=<YOUR_KUEUE_NAME>
export ARTIFACT_REGISTRY=<YOUR_ARTIFACT_REGISTRY_REPO_FULL_PATH> 
export GCS_BUCKET=<YOUR_GCS_BUCKET_FOR_STORING_LOGS>
export VLLM_IMAGE=vllm-arm64-gb200
export VLLM_VERSION=v0.14.0rc1
export NGC_VLLM_VERSION=25.12.post1-py3
...
| --------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- |
| `PROJECT_ID` | Your Google Cloud Project ID. | `google-cloud-project-12345` |
...
| `ARTIFACT_REGISTRY` | Full path to your Artifact Registry repository in the format of `<REGION>-docker.pkg.dev/<PROJECT_ID>/<ARTFIFACE_REGISTRY_REPO_NAME>`. | `us-central1-docker.pkg.dev/google-cloud-project-12345/my-repo` |
| `GCS_BUCKET` | Name of your GCS bucket (do not include `gs://`). | `my-benchmark-logs-bucket` |
| `VLLM_IMAGE` | The name for the Docker image to be built. | `vllm-arm64-gb200` |
| `VLLM_VERSION` | The version for [vLLM GitHub repo](https://github.com/vllm-project/vllm). | `v0.14.0rc1` |
| `NGC_VLLM_VERSION` | The version for NVidia's base vLLM image from [NGC](https://catalog.ngc.nvidia.com/orgs/nvidia/containers/vllm?version=25.12.post1-py3). |`25.12.post1-py3` |
...
This step builds a custom Docker image with vLLM on your local host machine (either non-ARM64 or ARM64 machine) and pushes it to your Artifact Registry repository as defined.

> [!NOTE]
> This build process can take **up to 30 minutes** as it compiles and installs several dependencies.

> [!NOTE]
> If you are building the `linux/arm64` image on a non-ARM host (e.g. an x86 AMD64 machine), you need to ensure your system is set up for cross-compilation using QEMU. This allows your host machine to emulate ARM64 execution. Run the following command on your non-ARM64 host machine to register QEMU user static handlers:
```
docker run --rm --privileged multiarch/qemu-user-static --reset -p yes
```

```bash
cd $REPO_ROOT/src/docker/vllm-arm64

docker buildx build --platform linux/arm64 \
-t "${ARTIFACT_REGISTRY}/${VLLM_IMAGE}:vllm${VLLM_VERSION}-ngcvllm${NGC_VLLM_VERSION}" \
-f vllm-arm64.Dockerfile \
--build-arg VLLM_VERSION=${VLLM_VERSION},NGC_VLLM_VERSION=${NGC_VLLM_VERSION} \
--push .
```
...
<a name="serving-deepseek-r1-671b"></a>
### 4.1. Serving and Inference Benchmarking DeepSeek-R1 671B Model
...
This recipe supports the deployment and runs inference benchmark for the [DeepSeek-R1 671B](https://huggingface.co/deepseek-ai/DeepSeek-R1) model using the vLLM framework on a single A4X node in native FP8 mode.

This recipe does the following steps to run the benchmarking:

1. Downloads the full DeepSeek-R1 model checkpoints from [Hugging Face](https://huggingface.co/deepseek-ai/DeepSeek-R1).
2. Loads the model checkpoints and applies vLLM optimizations.
3. Server is ready to respond to requests.
4. Runs the throughput and/or latency benchmarking.

<a name="deploy-deepseek-r1-671b"></a>
#### 4.1.1. Deploy DeepSeek-R1 671B Model
...
1.  **Make an API request:**
    ...
    "messages":[
        ...
        {
            "role":"user",
            "content":"How many r are there in the song title Strawberry Rush by Chuu ?"
        }
    ],
    "temperature":0.6,
    "top_p":0.95,
...
2.  **Stream a chat response:**
    ...
    (P.S. You might need to execute `chmod` to grant permission to read the `stream_chat.sh` file.)
...
#### 4.1.3. Benchmark DeepSeek-R1 671B Model
...
1.  Run the [vLLM benchmarking tool](https://docs.vllm.ai/references/benchmark_and_profiling.html) directly inside the running deployment:
      ...
      'mkdir -p /gcs/benchmark_logs/vllm && vllm bench serve \
        --model deepseek-ai/DeepSeek-R1 \
        --dataset-name random \
        --ignore-eos \
        --num-prompts 1100 \
...
```bash
# Example for DeepSeek-R1 671B Model
kubectl get deployment/$USER-serving-deepseek-r1-model
```
...
You should see logs indicating vLLM server downloading/loading the model, preparing the model, starting the API server, and then running the throughput benchmark test, similar to this:
