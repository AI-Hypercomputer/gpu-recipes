...
* **GKE Cluster**:
    ...
    * A GPU node pool with 1 [a4x-ultragpu-8g](https://cloud.google.com/compute/docs/gpus#h200-gpus) machine.
...
### 3.6. Build the TRTLLM Serving Image

This step uses Cloud Build to create a custom Docker image with TRTLLM and push it to your Artifact Registry repository.
...
1. Install the helm chart to prepare and benchmark the model using [`trtllm-bench`](https://github.com/NVIDIA/TensorRT-LLM/blob/main/docs/source/performance/perf-benchmarking.md) tool:
    ...
    --set-file serving_config=$REPO_ROOT/src/frameworks/a4x/trtllm-configs/llama-3.1-405b.yaml \
