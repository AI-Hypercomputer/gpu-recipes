...
echo "Dynamo vLLM launcher starting"
...
# Launch the Dynamo vLLM server
echo "Launching Dynamo vLLM server with model: $MODEL_NAME"
python3 -m dynamo.vllm \
  --model "$MODEL_NAME" \
  "$@"

echo "Dynamo vLLM server command finished."